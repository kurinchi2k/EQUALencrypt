...
*EQUALencrypt - Encrypt and decrypt whole files* [@Gurusamy:2025c] accepts whole files of any extension as input and uses openssl package to encrypt and digitally sign the file. To encrypt and digitally sign the file, it generates an unique pair of private and public RSA keys (4096 bits), encrypts the file using symmetric AES256 algorithm (32 bits for the key and 16 bits for initialization vector), encrypts the AES key using the asymmetric RSA keys, and includes padding according to PKCS #1 v2.0 specifications. It then inserts a digital signature using the SHA384 algorithm for the hash function.
